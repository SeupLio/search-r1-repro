...
| **7B/3B 真实 RL 训练（8×H100）** | ⚠️ **部分执行** | 本机有 RTX 3060（6GB）但驱动不稳定，只完成 3 步真实 GRPO 更新；见 §5.3 |
...
第二轮在本机 **NVIDIA GeForce RTX 3060 Laptop GPU（6GB）** 上做了两项实测。
结论先说：**其中一项推翻了我第一轮的估计。**

#### 5.3.1 E5 稠密检索 vs 稀疏 TF-IDF 的 recall@k 自检（实测，196 题）

实验设计：标准答案固定指向同一批黄金文档，只改变语料规模（往里塞合成干扰文档），
这样两次运行的差异**只**来自检索器本身。脚本 `analysis/recall_selfcheck.py`。

| 语料规模 | 稀疏 TF-IDF R@1 | 稀疏 R@3 | 稀疏 R@5 | E5 稠密 R@1 | E5 R@3 | E5 R@5 |
|---|---|---|---|---|---|---|
| 45 | 0.929 | **0.990** | 1.000 | 0.903 | 0.980 | 1.000 |
| 500 | 0.633 | 0.668 | 0.673 | **0.643** | **0.689** | **0.740** |
| 5000 | **0.617** | **0.658** | **0.673** | 0.628 | 0.648 | 0.648 |

两个必须写下来的结论：

1. **第一轮"top-3 召回 98.98%、检索几乎不漏"是 45 条微型语料造成的假象。**
   放到 5000 条规模，top-3 召回掉到 **0.65 左右**——检索实际漏掉约三分之一。
   这直接修正了 §7 归因里"retrieval_miss 只占 7.5%"的估计：**检索漏召回的真实占比要高得多**。
   原来以为瓶颈只在聚合推理，现在看检索同样是硬瓶颈。

2. **在元数据密集的中文短文本上，E5 并不比字符 n-gram TF-IDF 更好**（5000 条时
   稀疏反而略胜 0.658 vs 0.648）。原因是这类语料的判别信息集中在标题/平台/UP主等
   **字面精确匹配**的字段上，词法检索天然占优，而 E5 的语义泛化优势发挥不出来。
   → 这提示 §5.1 里"检索器不一致 -8~-3pt"这个估计**对本场景可能偏高**；
   但它只适用于本内容平台语料，换成 wiki 长文本结论未必成立，仍需真机验证。

#### 5.3.2 真机 GRPO 训练：链路已验证，但本机无法支撑长训练

`searchr1_repro/grpo_train.py` 在 RTX 3060 上完成了 3 个真实训练步
（Qwen2.5-0.5B-Instruct + LoRA，多轮 rollout + 检索 token mask + EM 奖励 + 分组相对优势）：

| step | train reward | loss | 平均检索次数 | 有学习信号的组 |
|---|---|---|---|---|
| 1 | 0.000 | 0.000 | 0.0 | 0 / 1 |
| 2 | 0.000 | 0.000 | 0.5 | 0 / 1 |
| 3 | **0.500** | **-0.982** | 0.5 | **1 / 1** |

step 3 的 `useful_groups=1` 是关键证据：组内 reward 出现了方差，
GRPO 优势非零、loss 非零、反向传播真的执行了——**整条 RL 闭环在真机 GPU 上跑通**。

**但必须说清楚为什么没有多 seed 长训练曲线**：本机 GPU 存在驱动级不稳定，
`model.generate` 在同一进程内连续调用约 6 次后必然硬崩（无 Python traceback，
换 dtype、关梯度检查点、开 KV cache、周期 empty_cache 均无效）。
多 seed × 数百步训练需要的调用量远超这个上限。因此：

- 训练器**代码已完成且逐步保存**崩溃前的真实数据（`--out` 每步增量落盘）；
- 多 seed 长训练需要换到稳定 GPU 环境（Linux + 数据中心卡，或本机重装驱动后再试）；
- 换环境后直接跑 `python -m searchr1_repro.grpo_train --seed 0/1/2 --steps 500` 即可，
  报告里的曲线与表格会自动被真实值替换。
...
| 优先级 | 事项 | 预期收益 |
|---|---|---|
| P0 | ~~对齐检索器 recall@k 自检~~ → **已完成**（§5.3.1） | 实测推翻"检索几乎不漏"，并发现 E5 在本场景不优于词法检索 |
| P0 | 真机多 seed 长训练，填 §5.3 的表 | **链路已验证（3 步真实更新），长训练需换稳定 GPU 环境** |
| P0 | 修复本机 GPU 驱动不稳定（generate 约 6 次必崩） | 重装/降级驱动后即可完成多 seed 训练 |
